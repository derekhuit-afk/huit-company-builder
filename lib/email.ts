...
export async function sendProposalEmail({ to, name, proposalId, companyName }: { to: string; name: string; proposalId: string; companyName: string }) {
  const url = `${process.env.NEXT_PUBLIC_BASE_URL || "https://huit.build"}/proposal/${proposalId}`;
  ...
  await resend.emails.send({
    ...
    html: wrap(`
      ...
      <div style="text-align:center;margin-top:24px">
        <a href="${process.env.NEXT_PUBLIC_BASE_URL || "https://huit.build"}/portal/{{clientId}}" style="display:inline-block;background:rgba(245,166,35,0.1);border:1px solid rgba(245,166,35,0.3);color:#F5A623;padding:12px 28px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">View Build Portal →</a>
      </div>`)
  });
}
